...
if opts.plotPatternDetection
    ...
    plot1 = plot(shoes.Left_HF(3,1:end-1),'r','lineWidth',1.5);
    hold on
    plot2 = plot(shoes.Right_HF(3,1:end-1),'Color',[0 0.498039215803146 0],'lineWidth',1.5);
    hold on
    plot3 = plot(tmp.meanVectt(1,1:end-1),'lineWidth',0.7,'LineStyle','--','Color',[0 0 0]);
    ...
    % T1)  only DS
    leg = legend([plot1,plot2,plot3,shadedPatch1], ...
        {'Left','Right','mean','doubleSupport'},'Location','northeast','FontSize',18);
    % if T1, removed from patternRanges SSrange variable before plots and
    % cut from the plots the last value plots with (3,1:end-1)
    % % T2)   DS + SS-right
    %      leg = legend([plot1,plot2,plot3,shadedPatch1,shadedPatch3], ...
    %         {'Left','Right','mean','doubleSupport','rightSupport'},'Location','northeast','FontSize',18);
    % % T3)   DS + SS-left
...
% save
if saveON
    save2pdf(fullfile(bucket.pathToPlots, ('T1_patternDetection')),fig,600);
end
